...
import { COMPANIES, fmtDateTime, fmtMoney, fmtNum, medBreakdown } from "../types";
...
interface Props {
  ...
  onCreatePurchase: (company: string, items: PurchaseItem[]) => Promise<Purchase | null>;
  onReceivePurchase: (p: Purchase) => Promise<void>;
}

export function Purchases({ medicines, purchases, pushToast, onCreatePurchase, onReceivePurchase }: Props) {
  ...
  const saveOrder = async () => {
    ...
    setSaving(true);
    const order = await onCreatePurchase(company, items);
    setSaving(false);
    if (!order) return;
    pushToast("success", `تم إنشاء الطلبية #${fmtNum(order.number)} من ${company} — بانتظار الاستلام`);
    setItems([]);
  };

  const receive = (p: Purchase) => {
    void onReceivePurchase(p);
  };
