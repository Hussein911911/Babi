...
interface Props {
  ...
  onImportBackup: (data: unknown) => Promise<boolean>;
...
export function Reports({ medicines, invoices, purchases, customers, restoreSeed, onImportBackup, pushToast }: Props) {
  ...
  const exportBackup = () => {
    download(
      JSON.stringify(
        { exportedAt: new Date().toISOString(), medicines, invoices, purchases, customers },
        null,
        2
      ),
    ...
    pushToast("success", "تم تنزيل النسخة الاحتياطية الكاملة (JSON)");
  };
