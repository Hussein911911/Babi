...
export function Login({
  ...
  const submit = async (e: FormEvent) => {
    ...
    setLoading(true);
    const err = await onLogin(username.trim().toLowerCase(), password);
    if (err) {
      setLoading(false);
      setError(err);
      setShake((s) => s + 1);
      ...
      <aside className="relative hidden overflow-hidden bg-gradient-to-bl from-slate-950 via-blue-950 to-cyan-900 text-white lg:block">
        ...
        <div className="relative flex h-full flex-col justify-between p-12">
          ...
          <p className="flex items-center gap-2 text-xs text-slate-400">
            ...
            جلسة آمنة — البيانات محفوظة على خادم النظام ومشتركة بين جميع الأجهزة
          </p>
        </div>
      </aside>
      ...
      <main className="flex items-center justify-center p-6 sm:p-10">
        <div className="w-full max-w-md">
              ...
              >
                {busy ? (
                  <>
                    ...
                    {bootstrapping ? "جارٍ تحميل بيانات النظام..." : "جارٍ التحقق..."}
                  </>
                ) : (
                  "دخول النظام"
                )}
          ...
          <div className="anim-fade-up mt-5 rounded-xl border border-dashed border-cyan-300/70 bg-cyan-50/60 p-4" style={{ animationDelay: "200ms" }}>
            ...
            <p className="mt-2.5 text-[11px] leading-5 text-cyan-800/70">
              غيّر كلمات المرور الافتراضية من صفحة الإعدادات قبل تسليم النظام.
            </p>
          </div>
        </div>
      </main>
