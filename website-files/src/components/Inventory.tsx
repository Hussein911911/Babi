...
interface Props {
  medicines: Medicine[];
  onSaveMedicine: (data: Omit<Medicine, "id" | "createdAt">, id?: string) => Promise<boolean>;
  onDeleteMedicine: (m: Medicine) => Promise<boolean>;
...
export function Inventory({ medicines, onSaveMedicine, onDeleteMedicine, pushToast, search, setSearch, category, setCategory, status, setStatus, cartItems, onAddToCart, goToSales }: Props) {
  ...
  const handleSave = async (data: Omit<Medicine, "id" | "createdAt">, id?: string) => {
    const ok = await onSaveMedicine(data, id);
    if (!ok) return;
    pushToast("success", id ? `تم تحديث بيانات «${data.name}» بنجاح` : `تمت إضافة «${data.name}» إلى المخزون`);
    setModalOpen(false);
    setEditing(null);
  };
  ...
  const confirmDelete = async () => {
    if (!toDelete) return;
    const ok = await onDeleteMedicine(toDelete);
    if (!ok) return;
    setToDelete(null);
  };
    ...
    <div className="space-y-4">
      ...
      <footer className="anim-fade-up flex flex-wrap items-center justify-between gap-3 text-[13px] text-slate-500">
        ...
        <span className="inline-flex items-center gap-1.5 text-slate-400">
          ...
          تُحفظ التغييرات فوراً على خادم النظام
        </span>
      </footer>
