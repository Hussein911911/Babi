...
interface Props {
  ...
  customers: Customer[];
  onAddCustomer: (payload: { name: string; kind: CustomerKind; phone: string; city: string }) => Promise<Customer | null>;
  ...
  onApproveInvoice: (inv: Invoice) => Promise<void>;
  onSettleInvoice: (inv: Invoice) => Promise<void>;
}
...
export function Sales({
  medicines, invoices, items, setItems,
  customers, onAddCustomer, onDeleteCustomer, pushToast, onPrintInvoice, userName,
  onCreateInvoice, onApproveInvoice, onSettleInvoice,
}: Props) {
  ...
  const quickAddCustomer = async () => {
    ...
    const c = await onAddCustomer({ name, kind: custTab, phone: "", city: "غير محددة" });
    if (!c) return;
    setCustomerId(c.id);
    setQuickName("");
    setCustOpen(false);
  ...
  const settle = (inv: Invoice) => {
    void onSettleInvoice(inv);
  };

  const approveInvoice = (inv: Invoice) => {
    void onApproveInvoice(inv);
  };

  const saveInvoice = async (mode: "print-approve" | "print-only" | "approve-only") => {
    setFormError("");
    ...
    if (mode === "print-approve") {
      pushToast("success", `تم اعتماد القائمة #${fmtNum(invoice.number)} وخصم الكميات — جاهزة للطباعة`);
      clearForm(); onPrintInvoice(invoice);
    } else if (mode === "print-only") {
      pushToast("info", `طُبعت القائمة #${fmtNum(invoice.number)} دون اعتماد — اعتمدها لاحقاً من السجل`);
      clearForm(); onPrintInvoice(invoice);
    } else {
      pushToast("success", `تم اعتماد القائمة #${fmtNum(invoice.number)} وخصم الكميات من المخزون`);
      clearForm();
    }
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <CustomersModal
        ...
        onAdd={onAddCustomer}
        onDelete={(id) => void onDeleteCustomer(id)}
      />
      {picker && (
        <MaterialPicker
