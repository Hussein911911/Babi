/* ================= البيانات التجريبية (بذرة النظام) =================
 * نفس البيانات المعروضة في واجهة النظام سابقاً — تُزرع في قاعدة البيانات
 * عند أول تشغيل، وعند استخدام «استعادة البيانات التجريبية».
 */

export const SEED_USERS = [
  { username: "admin", password: "1234", name: "أحمد الفيض", role: "مدير النظام" },
  { username: "staff", password: "1234", name: "سارة كريم", role: "موظفة مبيعات" },
];
...
function inv(number, days, hour, customerId, customer, payment, lines) {
...
function po(number, days, hour, company, received, lines) {
  const items = lines.map(([id, qty]) => {
    const m = SEED_MEDICINES.find((x) => x.id === id);
    return {
      medicineId: m.id,
      ...
      strength: m.strength,
      qty,
      cost: Math.round(m.buyPrice / Math.max(1, m.stripsPerPiece)),
    };
  });
  return {
    id: `seed-po-${number}`,
